...
fn run() -> Result<()> {
    ...
    loop {
        ...
        synthesize_chunk(&mut output, target_freq, &mut phase);
...
fn synthesize_chunk(buffer: &mut [i16], frequency: f32, phase: &mut f32) {
    if frequency <= 0.0 {
        buffer.fill(0);
        return;
    }

    let amplitude = i16::MAX as f32 * OUTPUT_GAIN;
